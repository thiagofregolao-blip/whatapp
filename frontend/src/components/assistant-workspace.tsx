...
export default function Assistant() {
  ...
  useEffect(() => { setVoiceHost(document.getElementById('luna-voice-host')) }, [])
  ...
  useEffect(() => {
    const choose = (event: Event) => { const id = (event as CustomEvent<string>).detail; if (id) api(`/api/whatsapp/messages/${encodeURIComponent(id)}`).then(m => { revision.current++; draftRef.current=null; setDraftState(null); setSelected(m); setReply('') }).catch(e => setError(e.message)) }
    window.addEventListener('luna-select',choose); return () => window.removeEventListener('luna-select',choose)
  }, [])
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setMessages(rows)
      {
        ...
        if (target && id !== linkedMessage.current) { revision.current++; draftRef.current = null; setDraftState(null); setSelected(target); setReply(''); linkedMessage.current = id }
      }
    }
  ...
  async function transcribe(m: Message) {
    setError(''); setNotice('Transcrevendo áudio…')
    try {
      const { transcript } = await api(`/api/whatsapp/messages/${encodeURIComponent(m.id)}/transcribe`, { method: 'POST' })
      const done = { ...m, transcript, transcript_status: 'done' }
      setSelected(old => old?.id === m.id ? done : old)
      setMessages(old => old.map(x => x.id === m.id ? done : x))
      setNotice(''); return done
    } catch (e: any) { setError(e.message); setNotice(''); return null }
  }
  async function findMessage(id: string) {
    const m = messages.find(item => item.id === id) || incoming.find(item => item.id === id)
    return m || await api(`/api/whatsapp/messages/${encodeURIComponent(id)}`)
  }
  ...
  async function voiceAction(name: string, args: any) {
    ...
    if (name === 'abrir_mensagem') {
      const read = m.media_type === 'audio' && !m.transcript ? (await transcribe(m)) || m : m
      if (!draftRef.current && !busy) selectMessage(read)
      return { ...read, next: 'Pergunte se o usuário quer preparar uma resposta. Não envie nada.' }
    }
    if (name === 'ouvir_audio') {
  ...
  return <main className="nexo-page assistant-page">
    ...
    <div className="grid lg:grid-cols-[340px_1fr] gap-5">
      ...
      <section className="space-y-4 min-w-0">
        ...
        {voiceHost && createPortal(<RealtimeVoice messageId={selected?.id} incoming={incoming} draft={draft} onAction={voiceAction} voiceEvent={voiceEvent} />, voiceHost)}
        ...
        {selected && <div className="rounded-2xl bg-[var(--nx-panel)] border border-[var(--nx-accent)]/30 p-4"><div className="flex justify-between gap-3"><h2 className="font-semibold">Selecionada: {selected.chat_name || selected.sender_name || selected.chat_id}</h2><button aria-label="Limpar seleção" onClick={() => selectMessage(null)}>✕</button></div><p className="text-xs text-[var(--nx-muted)] mt-1">Conversa: {selected.chat_id}</p><p className="whitespace-pre-wrap break-words my-3">{selected.media_type === 'audio' ? (selected.transcript ? `🎙 Transcrição: ${selected.transcript}` : selected.transcript_status === 'failed' ? 'Áudio recebido · a transcrição falhou' : 'Áudio recebido · ainda não transcrito') : selected.content || 'Mensagem sem texto'}</p>{selected.media_type === 'audio' && <div className="flex flex-wrap gap-2"><button className={button} onClick={() => playOriginal(selected)}>▶ Ouvir original</button>{!selected.transcript && <button className={button} disabled={busy} onClick={() => transcribe(selected)}>📝 Transcrever</button>}</div>}</div>}
        ...
        <div className="rounded-2xl bg-[var(--nx-panel)] border border-[var(--nx-line)] p-4"><h2 className="font-semibold mb-3">Converse com Luna</h2><p className="text-xs text-[var(--nx-muted)]">{selected ? 'Contexto: mensagem selecionada.' : 'Contexto: até 80 mensagens dos últimos 7 dias.'} Áudios novos são transcritos automaticamente.</p>
