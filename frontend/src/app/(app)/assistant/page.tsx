  ...
  return <main className="max-w-6xl mx-auto px-4 py-6 pb-16">
    ...
    <div className="grid lg:grid-cols-[340px_1fr] gap-5">
      ...
      <section className="space-y-4 min-w-0">
        <RealtimeVoice messageId={selected?.id} />
        ...
        <div className="rounded-2xl bg-[#131b2e] border border-white/10 p-4"><h2 className="font-semibold mb-3">Converse com Luna</h2><p className="text-xs text-slate-400">{selected ? 'Contexto: mensagem selecionada.' : 'Contexto: até 80 mensagens dos últimos 7 dias.'} Conteúdo de áudio não é transcrito nesta versão.</p>
          <div aria-live="polite" className="space-y-3 my-4 max-h-[430px] overflow-y-auto">{!turns.length && <p className="text-slate-400 py-6">Posso ajudar a encontrar pedidos, entender prioridades e sugerir uma resposta.</p>}{turns.map((t,i) => <div key={i} className={`p-3 rounded-xl whitespace-pre-wrap break-words ${t.role === 'user' ? 'bg-[#233d3a] ml-6' : 'bg-white/5 mr-3'}`}><p className="text-xs text-[#72d8c8] mb-1">{t.role === 'user' ? 'Você' : 'Luna'}</p>{t.content}</div>)}{busy && <p role="status">Processando…</p>}</div>
