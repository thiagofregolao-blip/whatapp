...
export default function Profile() {
  ...
  const [settings, setSettings] = useState<any>(null), [mode, setMode] = useState('personal'), [key, setKey] = useState(''), [theme, setTheme] = useState('dark'), [alerts, setAlerts] = useState('manual')
  const [busy, setBusy] = useState(false), [error, setError] = useState(''), [notice, setNotice] = useState('')
  async function load() { const value = await api('/api/assistant/status'); setSettings(value); setMode(value.mode) }
  useEffect(() => { load().catch(e => setError(e.message)); setTheme(localStorage.getItem('nexo-theme') || 'dark'); setAlerts(localStorage.getItem('nexo-alerts') || 'manual') }, [])
  function preference(name: string, value: string) { localStorage.setItem(`nexo-${name}`, value); window.dispatchEvent(new Event('nexo-preferences')); if (name === 'theme') setTheme(value); else setAlerts(value) }
  async function save() {
  ...
  return <main className="nexo-page profile-page"><div className="page-heading"><div><p className="eyebrow">DO SEU JEITO</p><h1>Seu perfil<span>.</span></h1></div></div>
    <section className="profile-summary"><Avatar name={user?.name || user?.email || 'Você'} /><div><h2>{user?.name || 'Sua conta'}</h2><p>{user?.email}</p></div><span className="small-badge">Nexo</span></section>
