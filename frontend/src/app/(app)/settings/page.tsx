...
export default function Profile() {
  ...
  return <main className="nexo-page profile-page"><div className="page-heading"><div><p className="eyebrow">DO SEU JEITO</p><h1>Seu perfil<span>.</span></h1></div></div>
    ...
    </section><div className="settings-stack"><section className="nexo-card"><h2>Conexão WhatsApp</h2><Link className="setting-row" href="/onboarding/connect"><Link2 /><span><strong>{session?.status === 'connected' ? 'WhatsApp conectado' : 'Conectar WhatsApp'}</strong><small>{session?.status === 'connected' ? 'Gerenciar dispositivo vinculado' : 'Conecte pelo computador'}</small></span><ChevronRight /></Link><Link className="text-link block mt-5" href="/diagnostics">Diagnóstico de conexão e voz</Link></section>
